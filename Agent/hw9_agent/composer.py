...
STAGE_CONFIG = {
    ...
    "critic": {
        ...
        "artifacts": ["analyzer.yaml", "plan.md", "template_plan.json"],
    },
    "assessment": {
        "prompt": "assessment.md",
        "skills": ["critic_checklist.md"],
        "artifacts": ["analyzer.yaml", "plan.md", "template_plan.json", "review.yaml"],
    },
}

CASE_ASSETS = ("blank_plan.json",)


def _load_case_assets(case_dir: Path) -> dict[str, dict]:
    """Load the staff-approved blank plan derived from the official JML."""
    assets: dict[str, dict] = {}
    for name in CASE_ASSETS:
        path = case_dir / name
        if not path.is_file():
            raise FileNotFoundError(f"Required teacher design asset is missing: {path}")
        payload = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(payload, dict):
            raise ValueError(f"Teacher design asset must be a JSON object: {path}")
        assets[name] = payload
    if assets["blank_plan.json"].get("status") != "teacher_approved":
        raise ValueError("blank_plan.json must be marked teacher_approved before running the pipeline")
    return assets
...
def compose_stage(
    ...
    if not method.jml:
        raise ValueError(
            f"{interface_name}.{method_name} has no embedded JML. "
            "Write and approve the complete staff contract before designing blanks."
        )
    assets = _load_case_assets(case_dir)
    sections = [
    ...
    sections.append("## Teacher-authored authoritative JML\n\n```jml\n" + method.jml + "\n```")
    sections.append(
        "## Teacher-approved blank plan (metadata, not a behavioural oracle)\n\n```json\n"
        + json.dumps(assets["blank_plan.json"], ensure_ascii=False, indent=2)
        + "\n```"
    )
    sections.append(
